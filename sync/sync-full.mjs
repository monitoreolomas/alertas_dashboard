 ...
 *   1. Trae todos los usuarios de la API Novit filtrando por appType
 *      (la API sin filtro tiene un bug y no devuelve todos los registros)
...
// Fetchear por segmento para evitar el bug de la API sin filtro.
// null se envía como el string "null" para el parámetro filter.
const APP_TYPES = ["android", "ios", "web", null];
...
function buildUrl(page, appType) {
  ...
  // appType null → filter={"appType":null}
  // appType "android" → filter={"appType":"android"}
  const filter = appType === null
    ? JSON.stringify({ appType: null })
...
async function fetchPage(page, appType, intentos = 3) {
  ...
  for (let i = 0; i < intentos; i++) {
    try {
      ...
      log(`  Página ${page} (${appType}) falló (intento ${i+1}/${intentos}): ${err.message}. Reintentando...`);
      await new Promise(r => setTimeout(r, 1500 * (i + 1)));
    }
  }
}

async function traerSegmento(appType) {
  const label = appType ?? "null";
  const first = await fetchPage(1, appType);
  log(`  [${label}] totalCount: ${first.totalCount.toLocaleString()}`);
  ...
  while (rows.length < first.totalCount) {
    ...
    log(`  [${label}] Fetched: ${rows.length.toLocaleString()} / ${first.totalCount.toLocaleString()}`);
    if (!gotAny || rows.length >= first.totalCount) break;
    page += PARALLEL_REQ;
  }

  log(`  [${label}] ✓ ${rows.length.toLocaleString()} usuarios`);
  return rows;
}

async function traerTodosDeAPI() {
  log("── PASO 1: Trayendo usuarios de la API (por appType) ──");

  const allRows = [];
  const seenIds = new Set();

  for (const appType of APP_TYPES) {
    const rows = await traerSegmento(appType);
    // Deduplicar entre segmentos por si algún usuario aparece en más de uno
    for (const r of rows) {
      if (!seenIds.has(r.id)) {
        seenIds.add(r.id);
        allRows.push(r);
  ...
  const byType = allRows.reduce((acc, r) => {
    const k = r.app_type || "null";
    acc[k] = (acc[k] || 0) + 1;
    return acc;
  }, {});
  log(`  Distribución appType: ${JSON.stringify(byType)}`);
  log(`Total API: ${allRows.length.toLocaleString()} usuarios`);
  return allRows;
}

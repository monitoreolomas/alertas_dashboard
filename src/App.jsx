  ...
  return (
    <div style={{marginBottom:8}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
        <span style={{fontSize:11,color:"#94a3b8",fontFamily:"'DM Mono',monospace"}}>{label}</span>
        <span style={{fontSize:11,color:"#475569",fontFamily:"'DM Mono',monospace"}}>{fmt(value)} <span style={{color:"#334155"}}>({pt}%)</span></span>
      </div>
      <div style={{height:5,background:"#0f172a",borderRadius:3,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${pv}%`,background:`linear-gradient(90deg,${color},${color}88)`,borderRadius:3,transition:"width 0.6s cubic-bezier(0.4,0,0.2,1)"}}/>
      </div>
    </div>
  );
}
...
function Heatmap({ matrix, rowLabels, colLabels }) {
  ...
  return (
    <div style={{overflowX:"auto"}}>
      <div style={{display:"grid",gridTemplateColumns:`48px repeat(${colLabels.length},1fr)`,gap:2,minWidth:400}}>
        <div/>
        {colLabels.map(l=><div key={l} style={{fontSize:8,color:"#334155",textAlign:"center",fontFamily:"'DM Mono',monospace",paddingBottom:3}}>{l}</div>)}
        {rowLabels.map((row,ri)=>(
          <>{/* eslint-disable-next-line react/jsx-key */}
            <div style={{fontSize:9,color:"#475569",display:"flex",alignItems:"center",fontFamily:"'DM Mono',monospace"}}>{row}</div>
            {colLabels.map((_,ci)=>{
              const v=matrix[ri][ci]; const intensity=v/max;
              return <div key={`${ri}-${ci}`} style={{background:`rgba(56,189,248,${0.04+intensity*0.86})`,borderRadius:2,aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,color:intensity>0.5?"#f0f9ff":"#334155",fontFamily:"'DM Mono',monospace",cursor:"default"}} title={`${row} ${colLabels[ci]}: ${v}`}>{v>0?v:""}</div>
            })}
          </>
        ))}
      </div>
...
function KPI({ label, value, sub, delta, sparkValues, color="#38bdf8", icon }) {
  const dNum=parseFloat(delta); const isPos=dNum>0; const isNeg=dNum<0;
  return (
    <div style={{background:"linear-gradient(145deg,#0d1b2e 0%,#111827 100%)",border:"1px solid #1a2744",borderTop:`2px solid ${color}`,borderRadius:10,padding:"18px 20px",position:"relative",overflow:"hidden"}}>
      <div style={{position:"absolute",top:0,right:0,width:70,height:70,background:`radial-gradient(circle at top right,${color}15,transparent 70%)`,pointerEvents:"none"}}/>
      <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",marginBottom:6}}>
        {icon&&<span style={{marginRight:5}}>{icon}</span>}{label}
      </div>
      <div style={{fontSize:30,fontWeight:700,color:"#f1f5f9",fontFamily:"'Syne',sans-serif",lineHeight:1,marginBottom:5}}>{value}</div>
      {sub&&<div style={{fontSize:10,color:"#475569",fontFamily:"'DM Mono',monospace",marginBottom:8}}>{sub}</div>}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        {delta!=null?<div style={{fontSize:10,fontFamily:"'DM Mono',monospace",color:isNeg?"#ef4444":isPos?"#34d399":"#475569",background:isNeg?"#ef444415":isPos?"#34d39915":"transparent",padding:"2px 7px",borderRadius:20,display:"inline-flex",alignItems:"center",gap:3}}>
          {isPos?"▲":isNeg?"▼":"●"} {Math.abs(dNum)}% vs anterior
        </div>:<div/>}
...
function Filters({ filters, setFilters, options }) {
  const inp = {background:"#080f1c",border:"1px solid #1a2744",color:"#94a3b8",borderRadius:7,padding:"6px 10px",fontSize:11,fontFamily:"'DM Mono',monospace",outline:"none",width:"100%"};
  return (
    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:24,alignItems:"flex-end"}}>
      {[["fechaDesde","Desde","date"],["fechaHasta","Hasta","date"]].map(([k,l,t])=>(
        <div key={k}><div style={{fontSize:9,color:"#334155",marginBottom:3,letterSpacing:"0.12em",fontFamily:"'DM Mono',monospace"}}>{l}</div>
        <input type={t} value={filters[k]} onChange={e=>setFilters(f=>({...f,[k]:e.target.value}))} style={inp}/></div>
      ))}
      ...
      <button onClick={()=>setFilters({fechaDesde:"",fechaHasta:"",cgm:"",categoria:"",tipo:""})}
        style={{background:"#0f172a",border:"1px solid #1a2744",color:"#475569",borderRadius:7,padding:"6px 12px",fontSize:10,fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>↺ Reset</button>
    </div>
  );
}
...
function ViewEjecutivo({ data, prevData }) {
  ...
  const topCats=topN(byCat,20); const catMax=Math.max(...topCats.map(([,v])=>v),1);
  ...
  const kc=["#38bdf8","#818cf8","#34d399","#fb923c","#f472b6","#facc15"];
  ...
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:28}}>
        <KPI label="Total Alertas" value={fmt(total)} sub={`${uniqueDays} días con registros`} delta={pct(total,totalPrev)} sparkValues={byDay.slice(-14)} color="#38bdf8" icon="📡"/>
        <KPI label="Promedio Diario" value={promDiario} sub="alertas por día activo" color="#818cf8" icon="📊"/>
        <KPI label="Hora Pico" value={`${String(horaPico).padStart(2,"0")}:00`} sub={`${byHour[horaPico]} alertas`} color="#fb923c" icon="⏰"/>
        <KPI label="Turno Líder" value={turnoMax?.[0]||"—"} sub={`${fmt(turnoMax?.[1])} · ${total>0?((turnoMax?.[1]/total)*100).toFixed(0):0}%`} color="#34d399" icon="🔄"/>
        <KPI label="Día Más Activo" value={DIAS[dowMax]} sub={`${fmt(byDow[dowMax])} alertas históricas`} color="#f472b6" icon="📅"/>
        <KPI label="Finde vs Hábil" value={`${total>0?((finde/total)*100).toFixed(0):0}% finde`} sub={`${fmt(finde)} finde · ${fmt(semana)} hábil`} color="#facc15" icon="🗓"/>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:16,marginBottom:16}}>
        <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px"}}>
          <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>DISTRIBUCIÓN POR CATEGORÍA</div>
          {topCats.map(([cat,val])=><HBar key={cat} label={cat} value={val} max={catMax} color={catColor(cat)} total={total}/>)}
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px",flex:1}}>
            <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>ORIGEN</div>
            {[["Sistema",bySistema,"#38bdf8"],["Botmarket",byBot,"#fb923c"]].map(([t,v,c])=><HBar key={t} label={t} value={v} max={Math.max(bySistema,byBot,1)} color={c} total={total}/>)}
          </div>
          <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px",flex:1}}>
            <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>POR TURNO</div>
      ...
      <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px"}}>
        <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>EVOLUCIÓN DIARIA</div>
        {dailyVals.length<2?<div style={{color:"#334155",fontSize:11}}>Sin datos</div>:(
          <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",height:110}}>
            <defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#38bdf8" stopOpacity="0.25"/><stop offset="100%" stopColor="#38bdf8" stopOpacity="0"/></linearGradient></defs>
            ...
            {pts.filter((_,i)=>i%step===0).map(([x,y],i)=>{
              ...
              return <g key={i}><circle cx={x} cy={y} r="2" fill="#38bdf8" opacity="0.7"/><text x={x} y={H-1} textAnchor="middle" fontSize="7" fill="#334155" fontFamily="DM Mono">{DIAS[d.getDay()]} {d.getDate()}</text></g>;
            })}
          </svg>
        )}
      </div>
...
function ViewTemporal({ data }) {
  ...
  return (
    <div>
      <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px",marginBottom:16}}>
        <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:4,fontFamily:"'DM Mono',monospace"}}>HEATMAP · DÍA DE SEMANA × HORA DEL DÍA</div>
        <div style={{fontSize:9,color:"#1e293b",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>Mayor intensidad = más alertas en ese cruce horario</div>
        <Heatmap matrix={heatmap} rowLabels={DIAS} colLabels={colHours}/>
      </div>

      <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px"}}>
        <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>SEMANA HÁBIL vs. FIN DE SEMANA · POR CATEGORÍA</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:12}}>
          {catFindeVsSem.map(([cat,{sem,fin}])=>{
            ...
            return (
              <div key={cat} style={{background:"#060d16",border:"1px solid #1a2744",borderRadius:8,padding:12}}>
                <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
                  <div style={{width:8,height:8,borderRadius:2,background:c,flexShrink:0}}/>
                  <span style={{fontSize:11,color:"#94a3b8",fontFamily:"'DM Mono',monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cat}</span>
                </div>
                ...
                <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"#475569",fontFamily:"'DM Mono',monospace"}}>
  ...
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1.6fr 1fr",gap:16,marginBottom:16}}>
        <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px"}}>
          <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>RANKING DE ZONAS · TOP 15</div>
          {topCgms.map(([cgm,val],i)=><HBar key={cgm} label={`${String(i+1).padStart(2,"0")}. ${cgm}`} value={val} max={cgmMax} color="#38bdf8" total={total}/>)}
        </div>
        <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px",overflowX:"auto"}}>
          <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>DETALLE POR ZONA</div>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:"'DM Mono',monospace"}}>
            <thead><tr style={{borderBottom:"1px solid #1a2744"}}>
              {["Zona","Total","Prom/día","Principal","H. Pico"].map(h=><th key={h} style={{textAlign:"left",padding:"4px 6px",color:"#334155",fontWeight:400,fontSize:9}}>{h}</th>)}
            </tr></thead>
            <tbody>
              {topCgms.map(([cgm,val])=>{
                const[catMain]=cgmCatPrincipal[cgm]||["—"]; const c=catColor(catMain);
                return <tr key={cgm} style={{borderBottom:"1px solid #060d16"}}>
                  <td style={{padding:"5px 6px",color:"#94a3b8",maxWidth:90,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cgm}</td>
                  <td style={{padding:"5px 6px",color:"#38bdf8"}}>{fmt(val)}</td>
                  <td style={{padding:"5px 6px",color:"#475569"}}>{cgmPromDiario[cgm]}</td>
                  <td style={{padding:"5px 6px"}}><span style={{background:`${c}20`,color:c,borderRadius:4,padding:"1px 5px",fontSize:9}}>{catMain}</span></td>
                  <td style={{padding:"5px 6px",color:"#fb923c"}}>{cgmHoraPico[cgm]!==undefined?`${String(cgmHoraPico[cgm]).padStart(2,"0")}:00`:"—"}</td>
                </tr>;
              })}
            </tbody>
          </table>
      ...
      <div style={{background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,padding:"18px 20px"}}>
        <div style={{fontSize:10,color:"#334155",letterSpacing:"0.14em",marginBottom:14,fontFamily:"'DM Mono',monospace"}}>COMPOSICIÓN POR CATEGORÍA · TOP 10 ZONAS</div>
        <div style={{display:"flex",flexDirection:"column",gap:7}}>
          {topCgms.slice(0,10).map(([cgm,tot])=>{
            ...
            return <div key={cgm} style={{display:"flex",alignItems:"center",gap:10}}>
              <span style={{fontSize:10,color:"#475569",width:110,flexShrink:0,fontFamily:"'DM Mono',monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cgm}</span>
              ...
              <span style={{fontSize:9,color:"#334155",width:36,textAlign:"right",fontFamily:"'DM Mono',monospace"}}>{fmt(tot)}</span>
            </div>;
          })}
        </div>
        <div style={{display:"flex",flexWrap:"wrap",gap:10,marginTop:14}}>
          {Object.entries(CAT_COLORS).filter(([k])=>k!=="default").map(([cat,c])=>(
            <div key={cat} style={{display:"flex",alignItems:"center",gap:5}}>
              <div style={{width:8,height:8,borderRadius:2,background:c}}/><span style={{fontSize:9,color:"#475569",fontFamily:"'DM Mono',monospace"}}>{cat}</span>
            </div>
          ))}
        </div>
      </div>
...
// ─── VIEW: MAPA ──────────────────────────────────────────────────────────────
function ViewMapa({ data, filters, setFilters, options }) {
  const mapRef = useRef(null);
  const leafletMapRef = useRef(null);
  const layersRef = useRef({});
  const [selectedZone, setSelectedZone] = useState(null);

  // Conteo por CGM con filtros aplicados (sin filtro de cgm para pintar todas)
  const countsByCgm = useMemo(() => groupBy(data, "cgm"), [data]);
  const maxCount = Math.max(...Object.values(countsByCgm), 1);

  // Color de relleno por intensidad
  function getZoneColor(count) {
    if (!count) return { fill: "#0a1221", opacity: 0.6 };
    const t = count / maxCount;
    // frío → caliente: azul oscuro → cyan → naranja → rojo
    if (t < 0.25) return { fill: `rgba(14,165,233,${0.2+t*1.2})`, opacity: 0.85 };
    if (t < 0.5)  return { fill: `rgba(56,189,248,${0.3+t*0.8})`, opacity: 0.88 };
    if (t < 0.75) return { fill: `rgba(251,146,60,${0.4+t*0.6})`, opacity: 0.9 };
  ...
  useEffect(() => {
    ...
    CGM_GEOJSON.features.forEach(feature => {
      ...
      layer.on("mouseover", function(e) {
        this.setStyle({ color: "#38bdf8", weight: 2.5, fillOpacity: Math.min(opacity + 0.1, 1) });
      });
      layer.on("mouseout", function() {
        this.setStyle({ fillColor: fill, fillOpacity: opacity, color: "#1e3a5f", weight: 1.5 });
      });
      layer.on("click", function() {
        setSelectedZone({ name, count });
        setFilters(f => ({ ...f, cgm: name }));
      });

      // label centrado
      const center = layer.getBounds().getCenter();
      const pct = count > 0 ? ((count / Object.values(countsByCgm).reduce((a,b)=>a+b,0))*100).toFixed(1) : "0";
      const label = L.divIcon({
        className: "",
        html: `<div style="font-family:'DM Mono',monospace;font-size:10px;color:#e2e8f0;text-align:center;white-space:nowrap;text-shadow:0 1px 3px #000,0 0 8px #000;pointer-events:none;"><div style="font-size:11px;font-weight:600;letter-spacing:0.05em">${name}</div><div style="font-size:9px;color:#94a3b8;">${count > 0 ? fmt(count)+" ("+pct+"%)" : "sin datos"}</div></div>`,
        iconAnchor: [0, 0],
      });
  ...
  const sortedZones = useMemo(() =>
    Object.entries(countsByCgm).sort((a,b)=>b[1]-a[1])
  , [countsByCgm]);
  ...
  return (
    <div style={{display:"flex",gap:16,height:"calc(100vh - 200px)",minHeight:500}}>
      {/* mapa */}
      <div style={{flex:1,position:"relative",borderRadius:10,overflow:"hidden",border:"1px solid #1a2744"}}>
        <style>{`.dark-tiles{filter:brightness(0.3) saturate(0.3) hue-rotate(195deg)}`}</style>
        <div ref={mapRef} style={{width:"100%",height:"100%"}}/>
        {/* legend */}
        <div style={{position:"absolute",bottom:40,left:12,background:"rgba(6,13,22,0.92)",border:"1px solid #1a2744",borderRadius:8,padding:"10px 14px",zIndex:999}}>
          <div style={{fontSize:9,color:"#334155",letterSpacing:"0.12em",marginBottom:7,fontFamily:"'DM Mono',monospace"}}>INTENSIDAD DE ALERTAS</div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:9,color:"#475569",fontFamily:"'DM Mono',monospace"}}>0</span>
            <div style={{width:100,height:7,borderRadius:3,background:"linear-gradient(90deg,#0e172a,#0ea5e9,#fb923c,#ef4444)"}}/>
            <span style={{fontSize:9,color:"#475569",fontFamily:"'DM Mono',monospace"}}>{fmt(maxCount)}</span>
          </div>
          {filters.cgm && (
            <button onClick={()=>setFilters(f=>({...f,cgm:""}))} style={{marginTop:8,width:"100%",background:"#1a2744",border:"none",color:"#94a3b8",borderRadius:5,padding:"3px 0",fontSize:9,fontFamily:"'DM Mono',monospace",cursor:"pointer"}}>
              ✕ Quitar filtro zona
            </button>
          )}
        </div>
      ...
      {/* ranking lateral */}
      <div style={{width:220,background:"#0a1221",border:"1px solid #1a2744",borderRadius:10,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"14px 16px 10px",borderBottom:"1px solid #1a2744",fontSize:9,color:"#334155",letterSpacing:"0.14em",fontFamily:"'DM Mono',monospace"}}>
          ...
          {sortedZones.map(([zone, count], i) => {
            ...
            return (
              <div key={zone} onClick={()=>setFilters(f=>({...f,cgm:isActive?"":zone}))}
                style={{padding:"7px 14px",cursor:"pointer",background:isActive?"#0f1f3d":"transparent",borderLeft:isActive?"2px solid #38bdf8":"2px solid transparent",transition:"all 0.15s"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                  <span style={{fontSize:10,color:isActive?"#e2e8f0":"#64748b",fontFamily:"'DM Mono',monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}}>
                    <span style={{color:"#1e3a5f",marginRight:5}}>{String(i+1).padStart(2,"0")}.</span>{zone}
                  </span>
                  <span style={{fontSize:10,color:"#38bdf8",fontFamily:"'DM Mono',monospace",marginLeft:6}}>{fmt(count)}</span>
                </div>
                <div style={{height:3,background:"#0f172a",borderRadius:2,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${pctVal/((sortedZones[0]?.[1]||1)/total)*100}%`,background:isActive?"#38bdf8":"#1e3a5f",borderRadius:2,transition:"width 0.4s"}}/>
                </div>
              </div>
            );
          })}
...
export default function App() {
  ...
  // En la vista mapa NO filtramos por CGM para poder pintar todas las zonas
  ...
  const tabStyle=(active)=>({
    background:active?"#0a1221":"transparent",
    border:"1px solid",borderColor:active?"#1a2744":"transparent",
    borderBottom:active?"2px solid #38bdf8":"2px solid transparent",
    color:active?"#f1f5f9":"#334155",
    borderRadius:"8px 8px 0 0",padding:"9px 18px",fontSize:10,
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        body{background:#040b14;}
        ::-webkit-scrollbar{width:5px;height:5px;}
        ::-webkit-scrollbar-track{background:#080f1c;}
        ::-webkit-scrollbar-thumb{background:#1a2744;border-radius:3px;}
        ::-webkit-scrollbar-thumb:hover{background:#253659;}
      `}</style>

      <div style={{minHeight:"100vh",background:"#040b14",color:"#e2e8f0",fontFamily:"'DM Mono',monospace"}}>

        {/* Header */}
        <div style={{borderBottom:"1px solid #0f1f3d",background:"linear-gradient(180deg,#071020 0%,#040b14 100%)",padding:"0 28px"}}>
          <div style={{maxWidth:1440,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:58}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:30,height:30,borderRadius:7,background:"linear-gradient(135deg,#0ea5e9,#6366f1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>⚡</div>
              <div>
                <div style={{fontSize:14,fontWeight:800,color:"#f1f5f9",fontFamily:"'Syne',sans-serif",letterSpacing:"0.04em"}}>CENTRO DE GESTIÓN MUNICIPAL</div>
                <div style={{fontSize:8,color:"#1e3a5f",letterSpacing:"0.22em"}}>ANÁLISIS OPERATIVO DE ALERTAS · LOMAS DE ZAMORA</div>
              </div>
            </div>
            <div style={{fontSize:9,color:"#1e3a5f",textAlign:"right"}}>
              <div style={{color:"#334155"}}>{loading?`Cargando… ${loadProgress.toLocaleString()} reg.`:`${allData.length.toLocaleString()} registros · ${filteredData.length.toLocaleString()} filtrados`}</div>
              <div style={{marginTop:2}}>{new Date().toLocaleDateString("es-AR",{day:"2-digit",month:"short",year:"numeric"})}</div>
            </div>
          </div>
        </div>

        {loading&&<div style={{height:2,background:"#080f1c"}}><div style={{height:"100%",background:"linear-gradient(90deg,#0ea5e9,#6366f1)",width:"55%",animation:"none",opacity:0.7}}/></div>}

        <div style={{maxWidth:1440,margin:"0 auto",padding:"24px 28px"}}>
          {error&&<div style={{background:"#1c0a0a",border:"1px solid #ef444433",borderRadius:8,padding:14,color:"#ef4444",fontSize:11,marginBottom:18}}>⚠ Error Supabase: {error}</div>}

          {/* Filters — sin el selector de CGM en el header (se controla desde el mapa) */}
          <Filters filters={filters} setFilters={setFilters} options={options}/>

          {/* Tabs */}
          <div style={{display:"flex",gap:3,marginBottom:24,borderBottom:"1px solid #0f1f3d"}}>
            {TABS.map(t=>(
              <button key={t.id} onClick={()=>setView(t.id)} style={tabStyle(view===t.id)}>
                <span style={{color:view===t.id?"#38bdf8":"#1e3a5f"}}>{t.icon}</span>
                {t.label.toUpperCase()}
              </button>
            ))}
            {filters.cgm&&(
              <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8,paddingBottom:2}}>
                <span style={{fontSize:9,color:"#334155",fontFamily:"'DM Mono',monospace"}}>ZONA:</span>
                <span style={{fontSize:10,color:"#38bdf8",fontFamily:"'DM Mono',monospace",background:"#0f1f3d",padding:"2px 10px",borderRadius:20}}>{filters.cgm}</span>
                <button onClick={()=>setFilters(f=>({...f,cgm:""}))} style={{background:"none",border:"none",color:"#334155",cursor:"pointer",fontSize:12,lineHeight:1}}>✕</button>
              </div>
            )}
          </div>
          ...
          {loading?(
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:280,gap:14}}>
              ...
              <div style={{fontSize:11,color:"#1e3a5f"}}>Cargando {loadProgress.toLocaleString()} registros…</div>
            </div>
          ):(
            <>
              ...
              {view==="mapa"&&(leafletReady
                ?<ViewMapa data={dataForMap} filters={filters} setFilters={setFilters} options={options}/>
                :<div style={{color:"#334155",fontSize:11,padding:20}}>Cargando mapa…</div>
              )}
            </>
          )}
        </div>
